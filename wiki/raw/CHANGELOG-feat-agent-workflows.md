...
### Changed
- Optimized the pull request CI pipeline (`ci.yml`) to selectively run `validate_devops`, `lint`, and matrix-based `test` jobs based on changed file paths, completely skipping runs on pure markdown/documentation-only PRs.
- Converted the CI `test` job to a matrix job that dynamically queries each bot's build and runpath dependencies via `go list`, running tests only on those packages.
