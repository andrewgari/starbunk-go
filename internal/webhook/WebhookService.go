...
func getWebhook(session *discordgo.Session, channelID string) (*discordgo.Webhook, error) {
	...
	if err1 != nil {
		log.Fatal("Could not find Channel with ID "+channelID, err1)
	}
	var channelName = fmt.Sprintf("Starbunk-Bot-%s", channel.Name)
	var webhooks, err2 = session.ChannelWebhooks(channelID)
	if err2 != nil {
		log.Fatal("Error getting Webhook for channel "+channelName, err2)
		return nil, err2
	}
	...
	if err3 != nil {
		log.Fatal("Error creating Webhook for channel "+channelName, err3)
		return nil, err3
	}
...
func WriteMessage(session *discordgo.Session, channelID string, content string, nickname string, avatarURL string) {
	...
	if err1 != nil {
		log.Fatal("Error Creating Webhook for channel "+channelID, err1)
	}
	...
	if errT != nil {
		log.Fatal("Error Getting Webhook I just created")
	}
	log.Default().Println(wh.ID)
	var _, err2 = session.WebhookExecute(webhook.ID, webhook.Token, false, &params)
	if err2 != nil {
		log.Fatal("Error Executing Webhook message: "+content+"\n", err2)
	}
}
