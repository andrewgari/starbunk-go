...
var _ = Describe("BunkBot e2e", func() {
	...
	BeforeEach(func() {
		harness = testenv.NewBotHarness("bunkbot-bot-id", auditor, messageCreate)
	})
	...
	DescribeTable("message routing",
		...
		Entry("self message is blocked by NotSelf", scenario{
			...
			msg: testenv.NewMessage(
				testenv.WithAuthorID("bunkbot-bot-id"), // matches harness botID
				testenv.WithGuildID(testenv.DefaultGuildID),
				testenv.WithContent("hello"),
			),
			wantAudit: false,
		...
		// BunkBot intentionally allows bot authors (no NotBot check).
		Entry("bot author passes audit (no NotBot policy)", scenario{
			desc: "bot author passes (BunkBot allows bots)",
			msg: testenv.NewMessage(
		...
		// BunkBot intentionally allows DMs (no GuildOnly check).
		Entry("DM passes audit (no GuildOnly policy)", scenario{
			desc: "DM passes (BunkBot allows DMs)",
			msg: testenv.NewMessage(
				testenv.WithAuthorID("user-123"),
				testenv.WithContent("hello"),
		...
		Entry("ping bunkbot triggers pong", scenario{
			desc: "ping bunkbot → Pong from bunkbot!",
			msg: testenv.NewMessage(
				testenv.WithAuthorID("user-123"),
				testenv.WithGuildID(testenv.DefaultGuildID),
				testenv.WithContent("ping bunkbot"),
			),
			wantAudit:   true,
			wantMsgs:    1,
			wantContent: "Pong from bunkbot!",
		}),
